...
//!
//! A node that has just exchanged heartbeats can want one of exactly four things: to
//! ask whether this node is awake, to come and be asked that itself, to be given the
//! file, or to trade what each of them knows. Anything else is refused by name rather
//! than guessed at, because a frame this build does not recognise is not something to
//! improvise on.
//!
...
use n333_core::presenting::{self, Signed as SignedPresenting};
...
pub enum Asked {
    ...
    /// A node that cannot be reached, come to be asked. It opened the connection
    /// because nobody could open one to it; if this node was drawn to ask it, the
    /// question goes back down the connection that is now open.
...
/// Read whatever the peer says next, and say which of them it is.
///
/// # Errors
    ...
    S: AsyncRead + Unpin,
{
    ...
    if let Ok(message) = presenting::open(&frame) {
        return Ok(Asked::Presenting(AsReceived { message, frame }));
    }
